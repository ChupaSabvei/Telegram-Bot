...
from src.bot.callbacks import safe_answer, safe_edit_message
...
async def browse_category(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    async with runtime.session_factory() as session:
        ...
        if user is None:
            ...
            return
        events = await event_repo.list_by_city_category(
            user.city_slug,
            category_slug,
        ...
        await safe_edit_message(callback, text=text, reply_markup=kb, parse_mode=EVENT_MESSAGE_PARSE_MODE)
...
async def back_to_menu(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    await safe_edit_message(
        callback,
        text=format_main_menu(user.city_slug),
